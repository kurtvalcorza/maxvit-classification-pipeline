...
from maxvit_classification_pipeline.pipeline import MANIFEST_NAME, MODEL_ID, MaxViTPipeline

_CONFIG = json.dumps({"architecture": "maxvit_tiny_tf_224", "pretrained_cfg": {"tag": "in1k"}}).encode()
...
def test_unpinned_package_refuses_before_model_imports(tmp_path, monkeypatch, forbid_model_imports):
    ...
    with pytest.raises(RuntimeError, match="no pinned revision"):
        MaxViTPipeline.from_pretrained(device="cpu", weights_dir=tmp_path)
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(
    ...
    with pytest.raises(FileNotFoundError, match="no snapshot manifest"):
        MaxViTPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
...
def test_from_pretrained_refuses_bad_class_names_before_model_imports(tmp_path, pinned, forbid_model_imports):
    ...
    with pytest.raises(ValueError, match="duplicates"):
        MaxViTPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, class_names=["a", "a"])
