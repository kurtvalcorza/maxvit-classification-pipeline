...
FILES = {
    "README.md": b"# card\n",
    "config.json": b'{"architecture": "maxvit_tiny_tf_224", "pretrained_cfg": {"tag": "in1k"}}',
    "model.safetensors": b"\x00" * 64,
}
REFERENCE = {"pytorch_model.bin": b"\x02" * 72}
...
def test_pin_writes_commit_digests_and_module_revision(tmp_path):
    ...
    manifest = json.loads((root / "weights/maxvit-tiny-tf-224-in1k/dimer-base-manifest.json").read_text())
    ...
    module = (root / "src/maxvit_classification_pipeline/pipeline.py").read_text()
...
def test_pin_refuses_a_digest_that_disagrees_with_the_hub(tmp_path):
    root = _copy_repo(tmp_path)
    before = (root / "weights/maxvit-tiny-tf-224-in1k/dimer-base-manifest.json").read_text()
    module_before = (root / "src/maxvit_classification_pipeline/pipeline.py").read_text()
    ...
    assert (root / "weights/maxvit-tiny-tf-224-in1k/dimer-base-manifest.json").read_text() == before
    assert (root / "src/maxvit_classification_pipeline/pipeline.py").read_text() == module_before
